...
- **タグの絞り込みが無い。** `data/tags.json`（タグマスタ）を取っていないため。
  ⚠ ただし `data/jobs.json` の求人には**タグが平均29個ずつ付いている**（146種類）。
  絞り込みにも求人詳細にも出していないので、いまは完全に眠っている。
  出すなら jobsite の `fetch-tags.js` ＋ タグの箱 ＋ `TAG_SKIP` を移植する
- 折りたたみの箱（`fgroup--fold`）が無い（タグが無いので要らない）
...
- 求人詳細のタグリンク（`?tag=…`）が無いので `tagFacetOf()` も無い
