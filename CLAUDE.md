...
流れは **Airtable「求人DB（企業）」の 従業員数 列 → `node fetch-employees.js` → `data/employees.json`
→ `node rebuild.js` の `attachEmployees()` が求人1件ずつに `employees`（原文）を差し込む
→ `template.html` の `empNum()` が人数を読み、`EMP_BANDS` の段に振り分ける**。ロゴと同じ建て付け。
...
- ⚠ **求人詳細には原文を出す。** 「連結3,039名、単体835名」「約37名（グループ含むと約180名）」のように
  ...
  読み取った数字は絞り込みとカードの段にだけ使う
- ⚠ **読めなかった企業は段に入れない**（「非公開（直近3年で約5倍に拡大）」の estie など）。
  企業規模で絞ると出ない。推測で段に入れると絞り込みの結果が事実と食い違う
- ⚠ **`data/jobs.json` に書かない。** ロゴと同じ理由で、jobs.json は取り直すたび丸ごと入れ替わる
- ⚠ **数値化は `template.html` 側に置いてある**（`empNum` / `EMP_BANDS`）。`employees.json` は原文のまま。
  段の切り方を変えるのに Airtable を取り直さなくて済むようにするため
- `empNum()` は最初に出てくる「n名／n人」を採る。**「役員4名 正社員75名」のような先頭は読み飛ばす**。
  全角数字・全角カンマも読む。「770万人」のように 万 をまたぐものは読まない
- 絞り込みの並びは件数順ではなく**人数の小さい順**。`facetDefs` に `order` を持たせた facet だけこの並びになる
- ⚠ **中途サイト（jobsite） に同じ仕組みが入っている。** `fetch-employees.js`・`attachEmployees()`・`empNum()`・`EMP_BANDS`・
  `data/employees.json` が鏡写し。片方だけ直すと、同じ企業が片方のサイトでだけ段に入る
- 2026-09-05時点の内訳（掲載24件すべてが段に入る。51〜100名 1 / 101〜300名 10 / 301〜1,000名 9 / 1,001名以上 4（〜50名は該当なしなので段そのものが出ない））。**未記入は Airtable の「従業員数」列を埋めれば増える**
